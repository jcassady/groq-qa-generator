...
def generate_qa_pairs(text_chunks, groq_config):
    ...
    api_key = groq_config["api_key"] if "api_key" in groq_config else get_api_key()
    ...
    # Write QA pairs to file using the updated text_processing.py
    write_response_to_file(
        response=qa_pairs,
        output_file=groq_config["output_file"],
        json_format=groq_config["json"],
    )
